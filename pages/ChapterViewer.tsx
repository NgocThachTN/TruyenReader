...
const ChapterViewer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const currentIndex = allChapters.findIndex(c => c.chapter_api_data === decodedUrl);
  ...
  const handleChapterChange = (chapter: ChapterInfo) => {
      if (chapter && slug) {
          navigate(`/chapter/${slug}/${encodeURIComponent(chapter.chapter_api_data)}`);
          window.scrollTo(0, 0);
  ...
  if (error || !data) {
    return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
            <div className="text-red-400 bg-red-500/10 p-4 rounded-lg">{error || 'Could not load chapter'}</div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col">
        {/* Control Bar */}
        <div className="sticky top-0 z-40 bg-slate-900/90 backdrop-blur border-b border-slate-800 py-2 px-4 flex gap-4 justify-between items-center">
            <div className="flex items-center gap-2 shrink-0">
                <button onClick={() => navigate(`/comic/${slug}`)} className="text-slate-300 hover:text-white flex items-center gap-2 p-2 hover:bg-slate-800 rounded-lg transition-colors">
            ...
            {/* Chapter Navigation */}
            <div className="flex items-center gap-1 sm:gap-4 bg-slate-800/50 rounded-xl p-1 border border-slate-700/50 relative">
                <button 
                ...
                <button 
                    ...
                    className="flex items-center gap-1 text-slate-200 font-medium text-sm px-2 hover:text-emerald-400 transition-colors max-w-[150px] sm:max-w-[250px]"
                >
                    <span className="truncate">
                        {data.item.chapter_path.split('/').pop()?.replace(/_/g, ' ') || 'Chapter'}
                    </span>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`w-4 h-4 transition-transform ${isChapterListOpen ? 'rotate-180' : ''}`}>
                    ...
                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-64 max-h-80 overflow-y-auto bg-slate-900 border border-slate-700 rounded-xl shadow-xl z-50 custom-scrollbar">
                        {comic.chapters[0]?.server_data.map((chapter, idx) => (
                            <button
                                ...
                                className={`w-full text-left px-4 py-3 border-b border-slate-800 hover:bg-slate-800 transition-colors flex justify-between items-center ${
                                    chapter.chapter_api_data === decodeURIComponent(apiUrl || '') 
                                        ? 'bg-emerald-500/10 text-emerald-400' 
                                        : 'text-slate-300'
                                }`}
                            >
                                <span className="font-medium">Chapter {chapter.chapter_name}</span>
                            </button>
                        ))}
                    </div>
                )}
            ...
            <div className="flex items-center gap-2 bg-slate-800 rounded-lg p-1 shrink-0">
                <button 
                    ...
                    className={`p-2 rounded-md text-sm font-medium transition-colors ${readingMode === 'scroll' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'}`}
                    title="Scroll Mode"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                ...
                <button 
                    ...
                    className={`p-2 rounded-md text-sm font-medium transition-colors ${readingMode === 'single' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'}`}
        ...
        <div className="flex-grow w-full max-w-4xl mx-auto bg-black min-h-screen">
            {readingMode === 'scroll' ? (
                ...
                // Single Page Mode
                <div className="flex flex-col items-center justify-center min-h-[80vh] p-4" onClick={(e) => {
        ...
        <div className="bg-slate-900 border-t border-slate-800 py-6">
            <div className="container mx-auto px-4 flex flex-col items-center gap-4">
                ...
                {comic && (
                    <Link to={`/comic/${comic.slug}`} className="text-slate-500 hover:text-emerald-400 text-sm mt-2">
                        Back to {comic.name}
                    </Link>
                )}
            </div>
